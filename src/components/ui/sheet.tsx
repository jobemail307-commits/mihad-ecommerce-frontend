...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "bg-primary/20 fixed inset-0 z-50 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
        ...
        {showCloseButton && (
          <SheetPrimitive.Close
            ...
            render={
              <Button
                variant="ghost"
                className="hover:bg-primary hover:text-primary-foreground absolute top-4 right-4 rounded-full"
                size="icon-sm"
              />
            }
          >
